...
N=100;      % the number of Fourier Cosine expansion terms
...
% n_v=200;                         % total nodes for log-variance. When N=252, J=155 is enough.
...
V_p_1=gather(real(exp(-r*Tmat)/(N_monitor+1)*ww.*Beta0_1*exp(-a*1i*omega)));
% V_p_2=gather(real(exp(-r*Tmat)/(N_monitor+1)*ww.*Beta0_2*exp(-a*1i*omega)))
